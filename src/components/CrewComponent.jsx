...
const CrewComponent = () => {
  ...
  const content = crew.map((content, index) => {
    return (
      ...
      >
        <h2 className="text-semiWhite text-[20px] ss:text-[28px] uppercase font-Bellefair">
          {content.role}
        </h2>
        ...
        <p
          className="text-babyBlue font-Barlow ss:px-18 md:px-0 text-base ss:text-[20px] md:text-[18px] md:w-[450px] 
        leading-[24px] ss:leading-[32px]"
        >
          {content.bio}
        </p>
      </div>
  ...
  const image = images.map((image, index) => {
    return (
      <div
        ...
        className={`${
          currentSlide === index ? "active-content" : "inactive-content"
        } px-24 ss:px-0 md:mr-24`}
      >
        <img
          src={image}
          className="w-full h-full border-b-[1px] border-semiWhite"
    ...
    <section id="crew">
      ...
      <div className="flex flex-col-reverse ss:flex-col items-center pb-12 ss:pb-0 md:flex-row md:pl-44 md:items-center gap-8">
        <div className="flex flex-1 flex-col-reverse ss:flex-col">
          ...
          <div className="flex w-full gap-4 ss:gap-6 mb-6 ss:my-12 justify-center md:justify-start">
            {page}
          </div>
        </div>
        {/* // RightDiv */}
